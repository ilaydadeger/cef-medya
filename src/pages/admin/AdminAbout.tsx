...
export default function AdminAbout() {
  ...
  const [aboutPageData, setAboutPageData] = useState<CmsData["aboutPage"]>(
    data.aboutPage,
  )
  ...
  const handleSectionChange = (
    section: "modernDesign" | "creativeApproach",
    field: string,
    value: string,
  ) => {
  ...
  const addBrand = () => {
    setAboutPageData((prev) => ({
      ...prev,
      brands: [...prev.brands, { image: "", name: "YENİ MARKA" }],
    }))
  }
        ...
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-white/40 font-light">
              Sayfa metinlerini, marka şeridini ve şirket değerlerini
              düzenleyin.
            </p>
          </div>
          <button
        ...
        <form className="space-y-12">
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            ...
            <div className="bg-[#0A0A0A] p-6 rounded-xl border border-white/5 w-fit">
              <FileUploadButton
                ...
                currentImg={aboutPageData?.heroBg}
                onUpload={(e) =>
                  handleFileUpload(e, "aboutHeroBg", (url) =>
                    setAboutPageData((prev) => ({ ...prev, heroBg: url })),
                  )
                }
              />
            </div>
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <h3 className="text-xl font-medium mb-6 text-cef-orange">
              1. Giriş / Modern Tasarım Alanı
            </h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">
                    Etiket (Örn: MODERN TASARIM)
                  </label>
                  <input
                    type="text"
                    value={aboutPageData?.modernDesign?.subtitle || ""}
                    onChange={(e) =>
                      handleSectionChange(
                        "modernDesign",
                        "subtitle",
                ...
                <div>
                  <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">
                    Devasa Başlık (Satır atlamak için Enter'a basın)
                  </label>
                  <textarea
                    rows={4}
                    value={aboutPageData?.modernDesign?.title || ""}
                    onChange={(e) =>
                      handleSectionChange(
                        "modernDesign",
                        "title",
              ...
              <div className="bg-[#0A0A0A] p-6 rounded-xl border border-white/5 self-start">
                <FileUploadButton
                  ...
                  currentImg={aboutPageData?.modernDesign?.image}
                  onUpload={(e) =>
                    handleFileUpload(e, "modernImg", (url) =>
                      handleSectionChange("modernDesign", "image", url),
                    )
                  }
                />
              </div>
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            ...
            <p className="text-white/50 text-sm mb-6">
              Logo şeridinin üzerinde yer alacak, ortalanmış video alanı. Lütfen
              sadece video (mp4, webm vb.) dosyası yükleyin.
            </p>
            <div className="bg-[#0A0A0A] p-6 rounded-xl border border-white/5 w-fit">
              <div className="flex flex-col gap-2">
                <label className="block text-xs uppercase tracking-widest text-white/50">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <label className="flex items-center justify-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg cursor-pointer transition-colors text-sm font-medium">
                    ...
                    <input
                      ...
                      className="hidden"
                      onChange={(e) =>
                        handleFileUpload(e, "pageVideo", (url) =>
                          setAboutPageData((prev) => ({
                            ...prev,
                            videoUrl: url,
                          })),
                      ...
                      disabled={uploadingField === "pageVideo"}
                    />
                  </label>
                  {aboutPageData?.videoUrl && (
                    <button
                      type="button"
                      onClick={() =>
                        setAboutPageData((prev) => ({ ...prev, videoUrl: "" }))
                      }
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <h3 className="text-xl font-medium mb-6 text-white/80">
              2. Referans Markalar (Kayan Şerit)
            </h3>
            <div className="space-y-4">
              {aboutPageData?.brands?.map((brand, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-4 bg-[#0A0A0A] p-4 rounded-xl border border-white/5"
                >
                  ...
                  <label className="text-xs uppercase tracking-widest text-white/50 hover:text-white cursor-pointer transition-colors bg-white/5 px-3 py-2 rounded">
                    ...
                    <input
                      ...
                      className="hidden"
                      onChange={(e) =>
                        handleFileUpload(e, `brand${idx}`, (url) =>
                          handleBrandChange(idx, "image", url),
                        )
                      }
                      disabled={uploadingField === `brand${idx}`}
                    />
                  </label>
                  <input
                    type="text"
                    value={brand.name}
                    onChange={(e) =>
                      handleBrandChange(idx, "name", e.target.value)
                    }
              ...
              <button
                type="button"
                onClick={addBrand}
                className="flex items-center gap-2 text-white/50 hover:text-white transition-colors text-sm font-medium mt-4"
              >
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <h3 className="text-xl font-medium mb-6 text-cef-turquoise">
              3. Yaratıcı Yaklaşım Alanı
            </h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-[#0A0A0A] p-6 rounded-xl border border-white/5 self-start order-2 lg:order-1">
                <FileUploadButton
                  id="creativeImg"
                  label="Sol Taraftaki Görsel"
                  currentImg={aboutPageData?.creativeApproach?.image}
                  onUpload={(e) =>
                    handleFileUpload(e, "creativeImg", (url) =>
                      handleSectionChange("creativeApproach", "image", url),
                    )
                  }
                />
              </div>
              <div className="space-y-6 order-1 lg:order-2">
                <div>
                  <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">
                    Büyük Başlık (Satır atlamak için Enter'a basın)
                  </label>
                  <textarea
                    rows={3}
                    value={aboutPageData?.creativeApproach?.title || ""}
                    onChange={(e) =>
                      handleSectionChange(
                        "creativeApproach",
                        "title",
                ...
                <div>
                  ...
                  <textarea
                    rows={5}
                    value={aboutPageData?.creativeApproach?.desc || ""}
                    onChange={(e) =>
                      handleSectionChange(
                        "creativeApproach",
                        "desc",
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <h3 className="text-xl font-medium mb-6 text-white/80">
              4. Ajans Değerleri (Alt Alan)
            </h3>
            <div className="grid md:grid-cols-3 gap-6">
              {aboutData.values.map((val, index) => (
                <div
                  key={index}
                  className="p-6 border border-white/10 rounded-xl bg-[#0A0A0A]"
                >
                  <div className="mb-4 text-xs font-bold tracking-[0.2em] text-white/30 uppercase">
                    Değer {index + 1}
                  </div>
