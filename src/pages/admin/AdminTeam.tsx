...
export default function AdminTeam() {
  ...
  const [teamPageData, setTeamPageData] = useState<CmsData["teamPage"]>(
    data.teamPage,
  )
  ...
  const addMember = () => {
    setTeamData((prev) => [
      ...prev,
      { name: "Yeni Üye", role: "Unvan", image: "" },
    ])
  }
        ...
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-white/40 font-light">
              Sayfa girişini ve ekip üyelerini düzenleyin.
            </p>
          </div>
          <button
            onClick={handleSave}
        ...
        <form className="space-y-12">
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            ...
            <div className="bg-[#0A0A0A] p-6 rounded-xl border border-white/5 w-fit">
              <FileUploadButton
                ...
                currentImg={teamPageData?.heroBg}
                onUpload={(e) =>
                  handleGeneralFileUpload(e, "teamHeroBg", (url) =>
                    setTeamPageData((prev) => ({ ...prev, heroBg: url })),
                  )
                }
              />
            </div>
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <h3 className="text-xl font-medium mb-6 text-cef-turquoise">
              1. Sayfa Girişi (Profesyonel Kadro)
            </h3>
            <div className="space-y-6">
              <div>
                <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">
                  Üst Başlık (Örn: PROFESYONEL KADRO)
                </label>
                <input
                  type="text"
                  value={teamPageData?.intro?.subtitle || ""}
                  onChange={(e) =>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  value={teamPageData?.intro?.title || ""}
              ...
              <div>
                ...
                <textarea
                  rows={5}
                  value={teamPageData?.intro?.desc || ""}
                  onChange={(e) => handleSectionChange("desc", e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-lg py-3 px-4 text-sm outline-none focus:border-cef-turquoise resize-none"
                />
              </div>
            </div>
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 rounded-2xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <p className="text-white/50 text-sm">
                Sayfanın alt kısmında listelenecek ekip üyeleri.
              </p>
              <button
                type="button"
                onClick={addMember}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {teamData.map((member, index) => (
                <div
                  key={index}
                  className="bg-[#0A0A0A] border border-white/10 p-6 rounded-xl relative group flex flex-col sm:flex-row gap-6"
                >
                  <button
                    type="button"
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <label className="text-[10px] uppercase tracking-widest text-white/50 cursor-pointer text-center hover:text-cef-orange transition-colors">
                        ...
                        <input
                          ...
                          disabled={uploadingField === `member${index}`}
                        />
                      </label>
                      {member.image && (
                        <button
