...
export default function AdminFaq() {
  ...
  const handleQuestionChange = (catIndex: number, qIndex: number, field: 'q' | 'a', value: string) => {
    setFormData(prev => {
      const newFaq = [...prev];
  ...
  const addCategory = () => {
    setFormData(prev => [...prev, { label: 'Yeni Kategori', items: [{ q: 'Soru?', a: 'Cevap...' }] }]);
      ...
      <div className="p-8 md:p-12 max-w-4xl">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-white/40 font-light">Sıkça sorulan soruları ve kategorileri düzenleyin.</p>
          </div>
          <button 
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="space-y-6">
            {formData.map((cat, catIndex) => (
              <div key={catIndex} className="bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden">
                <div 
                ...
                >
                  <div className="flex-1" onClick={(e) => e.stopPropagation()}>
                    <input 
                      type="text" value={cat.label} onChange={(e) => handleCategoryChange(catIndex, e.target.value)}
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      type="button" onClick={(e) => { e.stopPropagation(); removeCategory(catIndex); }}
                    ...
                    {expandedCat === catIndex ? <ChevronUp size={20} className="text-white/50" /> : <ChevronDown size={20} className="text-white/50" />}
                  </div>
                </div>
                  ...
                  <div className="p-6 pt-0 border-t border-white/5 bg-[#0A0A0A]/50">
                    <div className="space-y-6 mt-6">
                      {cat.items.map((item, qIndex) => (
                        <div key={qIndex} className="relative group pl-6 border-l-2 border-cef-turquoise/30 hover:border-cef-turquoise transition-colors">
                          <button 
                            type="button" onClick={() => removeQuestion(catIndex, qIndex)}
                          ...
                          <div className="grid gap-4 pr-8">
                            <div>
                              <label className="block text-[10px] uppercase tracking-widest text-white/50 mb-1">Soru</label>
                              <input 
                                type="text" value={item.q} onChange={(e) => handleQuestionChange(catIndex, qIndex, 'q', e.target.value)}
                            ...
                            <div>
                              <label className="block text-[10px] uppercase tracking-widest text-white/50 mb-1">Cevap</label>
                              <textarea 
                                value={item.a} onChange={(e) => handleQuestionChange(catIndex, qIndex, 'a', e.target.value)} rows={3}
                    ...
                    <button 
                      type="button" onClick={() => addQuestion(catIndex)}
